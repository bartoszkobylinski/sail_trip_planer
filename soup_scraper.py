...
def insert_title(title):
    database_conn = sqlite3.connect('url_and_data.db')
    with database_conn:
        cursor.execute("INSERT INTO Title VALUES (:Title)",{'Title':title})
        print(title + ' has been added to a database')
    database_conn.close()

def insert_paragraph(paragraph):
    database_conn = sqlite3.connect('url_and_data.db')
    with database_conn:
        cursor.execute("INSERT INTO Paragraph VALUES (:Paragraph)",{'Paragraph':paragraph})
        print(paragraph + ' has been added to a database')
    database_conn.close()
'''
links_list_level_0 = []
links_list_level_1 = []
...
def main_program(counter):
    while counter < 3:
        if counter == 0:
            ...
            links = list(set(links_list_level_1))    
            print(str(len(links_list_level_1)) + ' that is len level 1.')
            print(str(len(links)) + ' that is len level 1 after sorting.')
            counter +=1
        else:
            with open('level_2.txt', 'a') as file_l2:
                file_l2.write('aaaaaaaaaaaaaaaaa')
            for url in links:
                soup = parse_website(url)
                paragraph = find_paragraph(soup)
                title = find_title(soup)
...
print('Program has ended. Program has found ' + str(len(links)) + ' links.')
...
main_program(0)
#a = find_paragraph(parse_website(get_url_from_user(0)))
#print(a)